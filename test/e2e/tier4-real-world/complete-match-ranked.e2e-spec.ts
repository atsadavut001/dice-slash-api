...
describe('Tier 4: Complete Match Simulation — Ranked Mode (E2E)', () => {
  ...
  it('T4.1.1: complete Ranked match from queue join to HP <= 0 and Ranked reward payout', async () => {
    match = await setupTwoPlayerMatch(context.url, MOCK_TOKEN_1, MOCK_TOKEN_2, 'rank');
    expect(match.mode).toBe('rank');
    ...
    expect(res1.gemsEarned).toBeGreaterThanOrEqual(40);
    expect(res1.gemsEarned).toBeLessThanOrEqual(80);
    expect(res1.rankPointsChanged).toBe(100);
